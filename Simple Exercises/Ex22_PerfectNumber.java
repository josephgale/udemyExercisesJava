...
public class PerfectNumber {
   public static boolean isPerfectNumber(int number){
        ...
        int count = 1;
        int divisorSum = 0;
        while(count<number){//set counter to expire before it reaches the value of number
            if(number%count==0){//deterimine if a count is a divisor
                divisorSum += count;//add divisor to sum
                count = count + 1;
            } else {
                count = count + 1;
            }
