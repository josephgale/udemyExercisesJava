...
public class FirstLastDigitSum {
   public static int sumFirstAndLastDigit(int number) {
        ...
        int sumDigits = number % 10;//saves the one place digit

        while(number>=10){
            number = number /10;//reduces the original number to a single digit or 10;
        }

        sumDigits += number;//adds the first digit of the original number to the sumDigits variable
        return sumDigits;
    }
}
